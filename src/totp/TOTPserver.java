...
public class TOTPserver {
    ...
    public Datos postDatos(MultivaluedMap < String, String > datosParams) {
        ...
        System.out.println(compareTOTP(totpass)); //Show if it is correct
           
        return datos;                         
    }
    

    //Compare TOTP received from the APP with the TOTP generated by the sever
    public static String compareTOTP(String totpass)  {
    	...
    	if (totp.equals(totpass)){
    		...
    		resultado = "NOT valid. Server: " + totp + " - MyTOTP:  " + totpass;
    	}
    ...
    public static String generateTOTP()
    {
    	...
    	long T0 = 0;
    	long X = 30; //30 seconds interval
    	
        Date date = new Date();
        long milisec = date.getTime(); //Returns actual time in miliseconds since 1970
        long testTime[] = {milisec/1000L}; //Convert to seconds
        
        for (int i=0; i<testTime.length; i++) { //
        	long T = (testTime[i] - T0)/X;  //Interval X=30 sec
        	time = Long.toHexString(T);
        	}
        
		return generateTOTP1(key, time, returnDigits, "HmacSHA1");
    }
